...
# Check if private_key.pem, public_key.pem, and jwk.json already exist
if [[ ! -f "$PRIVATE_KEY_FILE" || ! -f "$PUBLIC_KEY_FILE" || ! -f "$JWK_FILE" ]]; then
  echo "Generating new RSA key pair and JWK file..."
...
# Extract the modulus from the public key
modulus=$(openssl rsa -in public_key.pem -pubin -modulus -noout | cut -d'=' -f2)

# Base64-url encode the modulus
# The following line is modified to ensure no line breaks in the output
modulus=$(echo -n "$modulus" | xxd -r -p | base64 | tr -d '\n' | tr '/+' '_-' | tr -d '=')
...
# Check if jwk.json already exists in JWK_DIRECTORY
JWK_FILE="${JWK_DIRECTORY}jwk.json"
if [[ ! -f "$JWK_FILE" ]]; then
  echo "Creating $JWK_FILE file..."
  # JSON Web Key Set content with modulus
  # Modified to ensure no line breaks or unnecessary spaces in the JSON file
  cat > "$JWK_FILE" << EOF
{
"keys": [
  {
